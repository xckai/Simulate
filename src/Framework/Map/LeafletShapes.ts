import L=require("leaflet")
	...
	L.SVG.include({
		...
		_updateStyle: function (layer) {
			var path = layer._path || layer._node,
				options = layer.options;
...
export	let  ImageMarker = SvgMarker.extend({
	...
	let imageMarker = function (latlng, option) {
		return new ImageMarker(latlng, option);
	};
export	let CircleMarker = SvgMarker.extend({
		options: {
			stroke: true,
			fill: true,
			type: "circle",
		...
		initialize: function (latlng, options) {
			L.Util.setOptions(this, options);
			this._latlng = L.latLng(latlng);
			if (options.radius_x === undefined) {this.radius_x = options.radius;}
			if (options.radius_y === undefined) {this.radius_y = options.radius;}
		}
	});
	let circleMarker = function (latlng, option?) {
		return new L.CircleMarker(latlng, option);
	};
export	let SquareMarker = SvgMarker.extend({
		options: {
			stroke: false,
			fill: true,
			type: "square",
	...
	let squareMarker = function (latlng, option) {
		return new SquareMarker(latlng, option);
	};
export	let TextMarker = SvgMarker.extend({
		options: {
			type: "text",
			style: "normal",
	...
	let textMarker = function (latlng, option) {
		return new TextMarker(latlng, option);
	};

export	let ArrowMarker = SvgMarker.extend({
		options: {
			type: "arrow",
			width: 10,
			height: 10,
